# -*- coding: utf-8 -*-
"""
Created on Sun Sep  4 17:29:23 2022
...
#%% Corrolate pan # to run order
run_order = pd.DataFrame()
run_order['Pan_#'] = Meta_data.loc[:,'Pan_#']
run_order['run_number'] = 0

for i in list(run_order.index.values) :
    if run_order.loc[i,'Pan_#'] == 1:
        run_order.loc[i,'run_number'] = 1
    if run_order.loc[i,'Pan_#'] == 11:
        run_order.loc[i,'run_number'] = 1
    if run_order.loc[i,'Pan_#'] == 3:
        run_order.loc[i,'run_number'] = 2
    if run_order.loc[i,'Pan_#'] == 13:
        run_order.loc[i,'run_number'] = 2
    if run_order.loc[i,'Pan_#'] == 5:
        run_order.loc[i,'run_number'] = 3
    if run_order.loc[i,'Pan_#'] == 15:
        run_order.loc[i,'run_number'] = 3
#%% Pickle that info
#pd.to_pickle(run_order,"pickle_jar/Run_order.pkl" )
...
# .corr is added to first take the correlation matrix of the dataframe
# then the correlation matrix is fed to the heatmap generator
...
X= dat[['MgO', 'Al2O3', 'SiO2', 'P2O5', 'Concentration(mM)', 'L/S', 'run_number']]
...
#Warning writes file
#mod_stat_df.to_csv("output_data/Unbound_water_V2_best_subset_linear_regression_and_C.csv")
...
#Warning writes file
#fig.savefig('output_data/Unbound_water_V2_best_subset_linear_regression.svg', transparent=False, bbox_inches="tight")
